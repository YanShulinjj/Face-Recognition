...
class Backend(QThread):
    ...
    def run(self):  # 在启动线程后任务从这个函数里面开始执行
        ...
        self.facenet.add_class(imgelist, person_name)
        self.finished.emit()
...
class Main_window(object):
    ...
    def add_windows(self):
        ...
        self.sign_window.confirm.clicked.connect(self.is_exits)
    ...
    def isblank(self):
        ...
        if not name:
            return True
        elif not self.new_window.male.isChecked() and not self.new_window.male.isChecked():
            return True
    ...
    def triggered(self):
        ...
        self.log.clicked.connect(self.show_table)
                ...
                if len(f) > 0:
                    ...
                    if len(pots) > 0 and self.sign_fg:
                        ...
                        if self.comman[self.finished] == 0 and eye(pots):
                            self.finished += 1
                            # print('眨眼：{} 次'.format(self.eye_times))
                        elif self.comman[self.finished] == 1 and mouth(pots):
                            self.finished += 1
                            # print('嘴巴张开：{} 次'.format(self.mouth_time))
                    try:
                        f = process_img(f)
                    except:
                        return
